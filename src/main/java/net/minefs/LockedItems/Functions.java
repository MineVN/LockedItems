...
public class Functions {
	...
	public static boolean haveLockedItems(Player p) {
		List<ItemStack> c = new ArrayList<>(Arrays.asList(p.getInventory().getContents().clone()));
		List<ItemStack> c2 = new ArrayList<>(Arrays.asList(p.getInventory().getArmorContents().clone()));
		c.addAll(c2);
		for (ItemStack i : c) {
